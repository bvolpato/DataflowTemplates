Parquet Files on Cloud Storage to Cloud Bigtable Template
---
A pipeline which reads data from Parquet files in Cloud Storage and writes it to Cloud Bigtable table.

:memo: This is a Google-provided template! Please
check [Provided templates documentation](https://cloud.google.com/dataflow/docs/guides/templates/provided/parquet-to-bigtable)
...
* **inputFilePattern** (Input Cloud Storage File(s)): The Cloud Storage location of the files you'd like to process. (Example: gs://your-bucket/your-files/*.parquet).
...
-DtemplateName="GCS_Parquet_to_Cloud_Bigtable" \
-pl v1 \
-am
```
...
```
Classic Template was staged! gs://<bucket-name>/templates/GCS_Parquet_to_Cloud_Bigtable
```
...
export TEMPLATE_SPEC_GCSPATH="gs://$BUCKET_NAME/templates/GCS_Parquet_to_Cloud_Bigtable"
...
gcloud dataflow jobs run "gcs-parquet-to-cloud-bigtable-job" \
  --project "$PROJECT" \
  --region "$REGION" \
...
-DbucketName="$BUCKET_NAME" \
-Dregion="$REGION" \
-DjobName="gcs-parquet-to-cloud-bigtable-job" \
-DtemplateName="GCS_Parquet_to_Cloud_Bigtable" \
