 ...
 * Apply {@link LocalSpannerIO#createTransaction()} transform, that lazily creates a transaction. The
...
public class LocalSpannerIO {

  private static final Logger LOG = LoggerFactory.getLogger(LocalSpannerIO.class);
  ...
  public abstract static class Read extends PTransform<PBegin, PCollection<Struct>> {
    ...
    public PCollection<Struct> expand(PBegin input) {
      ...
      checkArgument(
          ...
          "LocalSpannerIO.read() runs in a read only transaction and requires timestamp to be set "
      ...
      if (getReadOperation().getQuery() != null) {
        ...
        checkNotNull(
            ...
            "For a read operation LocalSpannerIO.read() requires a list of "
                + "columns to set with withColumns method");
        checkArgument(
            !getReadOperation().getColumns().isEmpty(),
            "For a read operation LocalSpannerIO.read() requires a non-empty"
                + " list of columns to set with withColumns method");
      } else {
        throw new IllegalArgumentException(
            "LocalSpannerIO.read() requires configuring query or read operation.");
      }
  ...
  /** Implementation of Read Rows Transform. */
    ...
    public PCollectionView<Transaction> expand(PInput input) {
      ...
      return collection
          .apply(
              "Create transaction",
              ParDo.of(
                  new LocalCreateTransactionFn(this.getSpannerConfig(), this.getTimestampBound())))
  ...
  /** Implementation of Write Rows Transform. */
        ...
        final PCollectionView<SpannerSchema> schemaView =
            schemaSeed
                .apply(
                    ...
                    ParDo.of(new LocalReadSpannerSchema(spec.getSpannerConfig(), dialectView))
  ...
  /** Implementation of Read Change Stream Transform. */
  @AutoValue
  public abstract static class ReadChangeStream
    ...
    public PCollection<DataChangeRecord> expand(PBegin input) {
      checkArgument(
          ...
          "LocalSpannerIO.readChangeStream() requires the spanner config to be set.");
      checkArgument(
          getSpannerConfig().getProjectId() != null,
          "LocalSpannerIO.readChangeStream() requires the project ID to be set.");
      checkArgument(
          getSpannerConfig().getInstanceId() != null,
          "LocalSpannerIO.readChangeStream() requires the instance ID to be set.");
      checkArgument(
          getSpannerConfig().getDatabaseId() != null,
          "LocalSpannerIO.readChangeStream() requires the database ID to be set.");
      checkArgument(
          getChangeStreamName() != null,
          "LocalSpannerIO.readChangeStream() requires the name of the change stream to be set.");
      checkArgument(
      ...
      checkArgument(
          ...
          "LocalSpannerIO.readChangeStream() requires the end time to be set. If you'd like to"
              + " process the stream without an end time, you can omit this parameter.");
      if (getMetadataInstance() != null) {
        checkArgument(
            getMetadataDatabase() != null,
            "LocalSpannerIO.readChangeStream() requires the metadata database to be set if metadata"
                + " instance is set.");
      }
  ...
  static class WriteToSpannerFn extends DoFn<Iterable<MutationGroup>, Void> {
    ...
    // LocalSpannerAccessor can not be serialized so must be initialized at runtime in setup().
  ...
  private LocalSpannerIO() {} // Prevent construction.
}
